...
> 更新（2026-07-19）：P0-1（Reviewer 严格只读）、P1-1（Create & start 语义与 E2E）、P1-2（smoke 传参）、P1-3（TS 工具链）、P1-4（逐 Review feedback draft）以及 P2-2 中前端去重/排序/内存上限部分已修复，相关内容已从本文移除；其余编号保持原状，故存在空缺。
...
2. **有几个会在真实使用中遇到的正确性问题**：历史页被 Git 实时状态绑死、base commit 之后已 commit 的文件在 Diff UI 中消失。
...
先把 Run 所有权、Git 历史/差异语义和输出脱敏修好，这个项目就会从“令人惊喜的原型”迈进“可以放心长期用的本地 Agent 工作台”。
