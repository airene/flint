...
> 更新（2026-07-19）：P1-1（Create & start 语义与 E2E）、P1-2（smoke 传参）、P1-3（TS 工具链）以及 P2-2 中前端去重/排序/内存上限部分已修复，相关内容已从本文移除；其余编号保持原状，故存在空缺。修复后 typecheck / test / build / e2e 四门全绿。

## 1. 结论先行

Flint 不像一个只写了一天的普通原型：产品边界清楚，共享契约、状态机、事件持久化、WebSocket replay、精确 session 恢复、Git snapshot、Fake CLI 和完整的人工 feedback gate 都已经成型。当前 147 项 Bun 测试全部通过，核心域模型的基础质量是好的。

但现在还不建议把它宣称为“安全可交付的 MVP”，原因有两类：

1. **两个核心安全不变量尚未真正成立**：Claude Reviewer 仍有写工作区的命令通道；服务崩溃/重复启动时也可能留下孤儿 Agent 进程并释放写锁。
2. **有几个会在真实多轮使用中遇到的正确性问题**：旧 feedback 文本可能被发给新 review、历史页被 Git 实时状态绑死、base commit 之后已 commit 的文件在 Diff UI 中消失。
...
| `bun test` | **147 pass / 0 fail** | 在允许 loopback 临时端口后，全部 22 个测试文件通过 |
...
| `bun run test:e2e` | 通过 8 pass / 0 fail（2026-07-19 修复后） | 用例已对齐 Create & start 自动启动语义 |
| Real CLI smoke | 未执行 | 脚本传参已修复；仍需人工授权后运行 |

## 4. 按严重度排序的问题

### P0-1：Claude Reviewer 的“只读”边界可被允许的 Git 命令绕过

**证据**

- `apps/server/src/drivers/cli-arguments.ts:30-39` 允许 `Bash(git diff *)`、`git log *`、`git show *` 等通配命令。
- `apps/server/src/drivers/cli-arguments.ts:87-103` 将这些规则作为 Claude Reviewer 的 `--allowedTools`；Claude 不像 Codex Reviewer 那样还有一层 OS/filesystem read-only sandbox。
- Git 的读命令并不都是无副作用的：`git diff --output=<path>` 会创建或覆盖文件，`--ext-diff` 还可调用 repository 配置的外部程序。本次已在临时 repository 实测：`git diff --output=review-wrote-this.patch` 确实产生了文件。

**影响**

“Reviewer 永远只读”是 Flint 最中心的安全承诺。当 repository 内容包含 prompt injection，或 Agent 生成了带写副作用的 Git 参数时，未提交文件可被覆盖。Review 后 snapshot 只能检测已经发生的改变，不能防止数据损坏。

**建议**

1. 首选是不向 Reviewer 暴露通用 Bash：由 Flint 服务端生成 status/diff/log 读取结果，Reviewer 只使用 `Read/Glob/Grep` 与受控的读取工具。
2. 如必须用 Git，提供 Flint-owned wrapper，只接受枚举的子命令和参数，强制 `--no-ext-diff --no-textconv`，禁止 `--output`、config override、alias 和任何可执行 hook/helper 的通道。
3. 再加一层真正的文件系统只读 sandbox，不把 CLI permission pattern 当作唯一安全边界。
4. 增加对抗测试：`--output`、`--ext-diff`、repository alias/config、软链接与带 shell metacharacter 的请求都必须无法修改 snapshot。
...
### P1-4：旧 Review 的 feedback 文本会沿用到新 Review，可能把错误的意见发回 Developer

**复现路径**

1. Review A 后生成/编辑 feedback A。
2. 发送 feedback A，Developer 完成修复。
3. 启动 Review B，它产生新 findings。
4. UI 现在会将 Review B 当作 `feedbackReviewRun`，但 textarea 仍是 feedback A；因文本非空，Send 按钮仍可用。

**证据**

- `apps/web/src/stores/task-workspace.ts:129-132` 只在整个 task `load()` 时清空 `feedbackText`。
- `apps/web/src/stores/task-workspace.ts:168-177` 新 review 结束后 refresh 会替换 runs/findings，但不根据 source review run 清空或重建 feedback draft。
- `apps/web/src/stores/task-workspace.ts:278-292` 发送时组合“当前最新 review ID + 当前 findings IDs + 旧 finalText”。
- `apps/web/src/components/FeedbackEditor.vue:28-29` 只要文本非空就可发送。

**建议**

- 将 feedback draft 建模为 `{ sourceReviewRunId, selectedFindingIds/fingerprint, text, dirty }`，而不是一个全局字符串。
- feedback source review 发生变化时，默认清空或重新生成；如旧文本有人工修改，用明确对话框让用户选择丢弃/保留，不能静默沿用。
- 增加“Review A → feedback → Review B”的 store 与 E2E 测试，并断言发送前 draft 所属 run 与 API source run 一致。
...
### P1-6：base commit 之后已提交的变更不会出现在文件列表中

**证据**

- `apps/server/src/services/git.service.ts:212-265` 从 `git status` 构建 `files`，所以它只知道当前 worktree/index 变化。
- `apps/server/src/services/git.service.ts:268-269,317-343` 的 aggregate patch 却是相对 task `baseCommit`。两者语义不一致。
- 本次临时 repository 复现：从 base 改文件并 commit 后，`trackedPatch` 包含改动，但 `status.clean === true` 且 `files === []`。Diff UI 因此显示 0 文件且无法逐文件查看。

**影响**

Flint 自己不会 commit，但用户、hook 或 Developer CLI 可能在任务期间 commit。此时 snapshot/reviewer 仍可看到 aggregate diff，人工 Diff UI 却看不到，造成审查证据不对称。

**建议**

- 文件 inventory 以 `baseCommit..working tree` 为统一语义：合并 `git diff --name-status -z <baseCommit>`、index/worktree status 与 untracked files，再计算最终状态。
- 增加“base 后有 commit、worktree clean”、“commit + 未提交二次修改”、“base 被 rebase/不可达”的 Git/API/UI 测试。

### P1-7：Agent 成功输出和 Review findings 绕过了现有脱敏边界

**证据**

- `EventService` 会在事件持久化前递归脱敏，失败路径的 `errorMessage` 也脱敏，这部分是对的。
- `apps/server/src/services/agent-run.service.ts:222-229` 却直接将 `result.finalMessage` 和 `result.structuredOutput` 写入 `agent_runs`。
- `apps/server/src/services/review.service.ts:143-174` 又从未脱敏 structured output 生成并持久化 finding title/description/suggestion。
- 因此 Agent 如果在最终回复或 finding 中引用了 repository 中的 token/secret，原值会进入 DB 并在 UI 展示，与 README 的广义“诊断输出在存储或展示前脱敏”表述不完全一致。

**建议**

- 将脱敏下沉到 AgentRun/Review 的持久化边界，覆盖 final message、structured output、findings 和 review parse failure payload。
- 如产品确实需要保留本地原始输出，应做成明确的选项（最好加密/限制文件权限），不要让同一条数据一份脱敏、一份明文。
- 用 bearer、`sk-*`、JSON credential key 和嵌套 finding 添加持久化层测试。
...
1. 收紧 Claude Reviewer，用真正的只读 sandbox/受控 Git adapter 取代 Bash 通配。
2. 引入 application owner + Run lease + parent-death 管理，然后做双实例和崩溃恢复测试。
3. 实际运行真实 CLI smoke（脚本传参已修复）；未获得人工授权前继续明确标记为未执行。

### 第 2 阶段：多轮工作流正确性

1. feedback draft 绑定 source review run，并持久化 feedback idempotency state。
2. 历史数据与实时 Git panel 解耦，Git 错误统一映射。
3. 统一 base-relative diff/file inventory，补齐 committed-change 场景。
4. 统一 AgentRun/Review 脱敏边界。
...
这两个选择不影响本次 review 结论，可以休息后再定（原第一个决策 —— Create 的语义 —— 已于 2026-07-19 定为“保留自动启动 + `Create & start` 显式文案”并落地）：

1. **原始 Agent 输出保留策略**：只存脱敏版，还是允许本地保留加密原文？
   我建议默认只存脱敏版，原文需显式 opt-in。
2. **下一个主线是可用性还是隔离性**：`@file`/Diff UX 还是 Worktree？
...
先把只读边界、Run 所有权和多轮 feedback 修好，这个项目就会从“令人惊喜的原型”迈进“可以放心长期用的本地 Agent 工作台”。
