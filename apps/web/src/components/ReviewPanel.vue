...
const props = defineProps<{ run: AgentRun | null; findings: ReviewFinding[]; busy?: boolean; readOnly?: boolean; stale?: boolean }>();
...
const counts = computed(() => ({
  P0: props.findings.filter((item) => item.severity === "P0").length,
  P1: props.findings.filter((item) => item.severity === "P1").length,
  P2: props.findings.filter((item) => item.severity === "P2").length,
}));
...
<template>
  <section class="panel review-panel">
    <header class="panel-header">
      <h2 class="panel-title">Claude Review</h2>
    ...
    <template v-if="run">
      ...
      <div v-if="run.reviewParseStatus === 'failed'" class="parse-warning">Structured output could not be parsed. The raw reviewer response remains available in the Claude panel.</div>
      <div v-if="findings.length" class="finding-list">
        <article v-for="finding in findings" :key="finding.id" :class="['finding', { dismissed: finding.dismissed }]">
      ...
      <div v-else-if="run.status === 'completed' && run.reviewParseStatus !== 'failed'" class="empty-state"><strong>No findings</strong><span>Claude did not report actionable issues.</span></div>
    ...
    <div v-else class="empty-state"><strong>No review yet</strong><span>Start a read-only Claude review when development is ready.</span></div>
  </section>
</template>
