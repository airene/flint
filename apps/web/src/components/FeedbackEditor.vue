...
const props = defineProps<{ findings: ReviewFinding[]; text: string; busy?: boolean; stale?: boolean }>();
...
<template>
  <section class="panel feedback-panel">
    <header class="panel-header">
      <h2 class="panel-title">Feedback to Codex <span class="badge">{{ selected.length }} selected</span></h2>
    ...
    <div class="panel-body feedback-body">
      <p class="help">Nothing is sent automatically. Review the selected findings and edit this message before resuming the exact Codex session.</p>
      ...
      <div class="feedback-actions">
        ...
        <button v-else class="button primary" :disabled="busy || !text.trim()" @click="emit('send')">Resume Codex session →</button>
      </div>
    </div>
  </section>
</template>
