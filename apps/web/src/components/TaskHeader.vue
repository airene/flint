...
const props = defineProps<{ task: Task; runs: AgentRun[]; busy?: boolean; codexReady?: boolean; claudeReady?: boolean }>();
const emit = defineEmits<{ develop: [prompt?: string]; review: []; cancel: [runId: string]; complete: [] }>();
...
<template>
  <header class="task-header">
    <div>
      <div class="task-heading-row"><span :class="['badge', statusClass]">{{ task.status.replaceAll('_', ' ') }}</span><span class="mono task-id">{{ task.id.slice(0, 8) }}</span></div>
      <h1>{{ task.title }}</h1>
...
<style scoped>
.task-header{display:flex;align-items:flex-start;justify-content:space-between;gap:28px;margin-bottom:22px}.task-header>div:first-child{max-width:760px}.task-heading-row{display:flex;align-items:center;gap:8px;margin-bottom:8px}.task-id{color:var(--faint);font-size:9px}.task-header h1{font-size:23px;margin-bottom:7px}.task-action-stack{display:grid;justify-items:end;gap:8px;min-width:360px}.continuation-composer{display:grid;grid-template-columns:minmax(220px,1fr) auto;gap:7px;width:100%}.task-actions{justify-content:flex-end;padding-top:3px}
</style>
